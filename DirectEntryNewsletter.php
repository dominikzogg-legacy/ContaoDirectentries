    ...
    public function prepare()
    {
        ...
        if(in_array('newsletter', $this->Config->getActiveModules()))
        {
            ...
            if($this->User->isAdmin || ($this->User->hasAccess('newsletter', 'modules') && isset($this->User->newsletters) && is_array($this->User->newsletters)))
            {
                ...
                $objNewsletter = $this->Database->query("SELECT * FROM tl_newsletter_channel ORDER BY title");
                ...
                if($objNewsletter->numRows)
                {
                    ...
                    while($objNewsletter->next())
                    {
                        ...
                        if($this->User->isAdmin || in_array($objNewsletter->id, $this->User->newsletters))
                        {
                            ...
                            $arrDirectEntry[$intCounter]['icons']['page']['icon'] = 'system/modules/newsletter/html/icon.gif';
